...
@responses.activate
def test_pre005_bound_subscription_not_found_warns():
    # PayG linked but the bound Azure subscription returns 404 (deleted or not
    # visible to this sign-in) -> a targeted "not found" WARNING, not the
    # generic "Unable to check PayG configuration" error from a raised HTTPError.
    from flightcheck.checks.prerequisites import run_prerequisites_checks
    ...
    responses.add(**pp.list_policy_environments(
        policy_id=pp.MOCK_POLICY_ID, environments=[pp.billing_policy_environment()]))
    responses.add(**arm.get_subscription(status=404))
    ...
    # The linkage still exists; 404 is ambiguous (deleted vs. cross-tenant), so
    # the cross-check flag stays True (not provably dead).
    assert runner._payg_configured is True
...
def test_pre005_subscription_warned_is_warning():
    ...
    # A Warned subscription still bills, so the PayG linkage must register as
    # configured for the cross-check ("linked but unhealthy", not "no PayG at
    # all").
    assert runner._payg_configured is True
