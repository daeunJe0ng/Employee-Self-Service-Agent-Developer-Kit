...
"""Tests for WD-WF-CAT-001 minimalBots Workday topic inventory."""
...
@responses.activate
def test_workday_topic_schema_names_pass(minimalbots_client) -> None:
    ...
    assert "WorkdayGetVisas" in r.result
...
@responses.activate
def test_servicenow_only_components_fail_workday_topic_inventory(
    minimalbots_client,
) -> None:
    ...
    assert "does not expose the expected Workday topic inventory" in r.result
...
def test_simplified_install_still_skips() -> None:
    runner = SimpleNamespace(minimalbots=None, config={})
