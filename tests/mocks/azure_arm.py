...
def get_subscription(
    *,
    ...
    Pass ``status=403`` (or 401) to model a permission-denied response, or
    ``status=404`` to model a deleted / not-visible subscription; the client
    maps each to a ``{"_error": ...}`` sentinel.
    """
    ...
    if status == 404:
        return {
            "method": "GET",
            "url": url,
            "json": {"error": {"code": "SubscriptionNotFound",
