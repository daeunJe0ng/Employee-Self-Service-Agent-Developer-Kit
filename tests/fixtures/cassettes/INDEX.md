...
| **Copilot Studio minimalBots PPAPI** (`/copilotstudio/minimalBots/api/{cdsBotId}/components`, `/copilotstudio/minimalBots/alm/{cdsBotId}/configure`, `/copilotstudio/minimalBots/alm/{cdsBotId}/export`) | `validated` | Live cassettes `flightcheck_minimalbots_components.yaml` (components, 200) and `flightcheck_minimalbots_alm.yaml` (ALM configure, 200 opted + 400 not-opted). Contract also in `C:\Users\dawnjeong\knowledge\ess-da-migration\documents\swagger.json` + auth handoff `...\AgentBuilder-Test-Agent-API-Auth-Handoff-2026-09-01.md`. | Per-environment PPAPI host, distinct from global `api.powerplatform.com`. Recorded from prod-alm-hr-import (TEST ring); `export` (binary zip) not yet cassetted. |
...
|---|---|---|---|
| minimalBots PPAPI | `POST /copilotstudio/minimalBots/api/{cdsBotId}/components?api-version=2024-10-01` (component snapshot — `connectionReferenceChanges[]` for G1/G2, `botComponentChanges[]` for G3). GOOD agent: 2 conn refs / 79 comps; ServiceNow-only agent: 1 conn ref / 76 comps | 200 | `flightcheck_minimalbots_components.yaml` |
| minimalBots PPAPI (ALM) | `GET /copilotstudio/minimalBots/alm/{cdsBotId}/configure?realm={Dev\|Test\|Prod}&api-version=2024-10-01` (ALM-opted agent — `grsRepositoryId` + `commitSha` + `values` for G4 GRS presence) | 200 | `flightcheck_minimalbots_alm.yaml` |
| minimalBots PPAPI (ALM) | `GET /copilotstudio/minimalBots/alm/{cdsBotId}/configure?realm=...` (non-ALM-opted agent — `ErrorCode 4003` "not opted into ALM", the G4 FAIL path) | 400 | `flightcheck_minimalbots_alm.yaml` |
| minimalBots PPAPI (ALM) | `POST /copilotstudio/minimalBots/alm/{cdsBotId}/export?api-version=2024-10-01` (ALM package .zip, G4 PUB-001). Binary body replaced by a **synthetic stand-in zip** before write — never the real package | 200 | `flightcheck_minimalbots_publishing.yaml` |
| minimalBots PPAPI (ALM) | `POST /copilotstudio/minimalBots/alm/import?api-version=2024-10-01` (multipart package upload, G4 PUB-002). Same-env package re-import collides on its own schema; no agent created, zero residue. Request package bytes replaced by a synthetic placeholder | 409 | `flightcheck_minimalbots_publishing.yaml` |
