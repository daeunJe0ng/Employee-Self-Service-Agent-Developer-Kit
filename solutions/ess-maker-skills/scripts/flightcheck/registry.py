...
_SPECS: list[CheckpointSpec] = [
    ...
    # ESS-SOLN-001: the base ESS agent package is present in the target env.
    # DA re-point validation reads minimalBots ALM GetConfigure GRS state
    # instead of the Dataverse solutions table.
    CheckpointSpec(
        key="ESS-SOLN-001",
        category_fn=run_solution_checks,
        category_label="Solution",
        clients=frozenset({PP_ADMIN, MINIMALBOTS}),
        requires_config=True,
        requires_dataverse_endpoint=True,
        prereqs=("ENV-002",),
        priority=Priority.CRITICAL.value,
