...
def run_prerequisites_checks(runner) -> list[CheckResult]:
    ...
    if pp is None:
        ...
        try:
                ...
                for p in enabled:
                    ...
                    if any(
                        ...
                        if sub_id:
                            ...
                            # environmentId is the schema's key field and is
                            # always present, so the linked count is just the
                            # number of rows the policy returned.
                            linked_env_count = len(envs)
                            break
            ...
            if perm_error:
                ...
                # The PayG linkage exists (a billing policy with a bound Azure
                # subscription is attached to this environment). Treat that as
                # "PayG configured" up front; only a subscription we can prove is
                # dead (Disabled/Deleted) revokes it below. This keeps a Warned/
                # PastDue (or health-unverifiable) subscription — which still
                # bills — from being read as "no PayG at all" by a cross-check.
                ...
                if arm is None:
                    ...
                    if isinstance(sub, dict) and sub.get("_error") == "not_found":
                        payg_result = CheckResult(roles=[Role.POWER_PLATFORM_ADMIN.value],
                            checkpoint_id="PRE-005", category="Prerequisites",
                            priority=Priority.HIGH.value, status=Status.WARNING.value,
                            description="Pay-as-you-go billing configured (if needed)",
                            result=f"This environment is {_payg_plan_scope(name, linked_env_count)}, but its bound Azure subscription {linked_sub_id} was not found — it may have been deleted, or it is in a tenant this sign-in cannot see.",
                            remediation=f"Confirm Azure subscription {linked_sub_id} still exists and is the one intended for Pay-as-you-go in {payg_portal}; if it was deleted, link a current subscription, then re-run.",
                            doc_link=payg_doc,
                        )
                    elif isinstance(sub, dict) and "_error" in sub:
                        payg_result = CheckResult(roles=[Role.POWER_PLATFORM_ADMIN.value],
                            checkpoint_id="PRE-005", category="Prerequisites",
                            priority=Priority.HIGH.value, status=Status.WARNING.value,
                            description="Pay-as-you-go billing configured (if needed)",
                        ...
                        if state_norm == "enabled":
                            ...
                            if isinstance(budgets, list) and budgets:
                                ...
                                payg_result = CheckResult(roles=[Role.POWER_PLATFORM_ADMIN.value],
                                    ...
                                    remediation=f"Hardening recommendation (not a functional blocker): PayG usage bills directly to Azure with no cap, so a runaway consumption spike goes unchecked. Configure a budget/cost alert on subscription {linked_sub_id} in the [Azure Cost Management portal](https://portal.azure.com/#view/Microsoft_Azure_CostManagement), then re-run.",
                                    doc_link=payg_doc,
                                )
                            else:
                                payg_result = CheckResult(roles=[Role.POWER_PLATFORM_ADMIN.value],
                            ...
                            # A dead subscription cannot bill, so revoke the
                            # linkage-level flag: a cross-check must treat zero
                            # allocation here as a hard FAIL, not a
                            # surprise-billing WARNING.
                            payg_configured = False
                            payg_result = CheckResult(roles=[Role.POWER_PLATFORM_ADMIN.value],
                                checkpoint_id="PRE-005", category="Prerequisites",
    ...
    if payg_result is not None:
        ...
        # True whenever a PayG billing policy with a bound subscription is linked
        # to this environment AND that subscription is not provably dead
        # (Disabled/Deleted) — i.e. "PayG provides a billing path". A Warned/
        # PastDue or health-unverifiable subscription still counts (it bills), so
        # a future consumer never misreads "PayG linked but unhealthy" as "no
        # PayG at all". Set on the PayG arm only (not the prepaid arm). Read it
        # via getattr(runner, "_payg_configured", None) and treat True as "PayG
        # covers billing" (AGENTS.md principle #11). TODO: no consumer yet —
        # PRE-004 / PRE-006 are not implemented.
