...
**Enabled/disabled state (`_apply_state`).** A topic's `state`/`status` lives *beside* the payload, not inside it, so a structural payload merge never sees it. The DA template ships each topic with a state; if the customer set a different one — e.g. disabling the `ConversationStart` greeting the template itself suggests turning off — that is a real customization. `_merge_one` compares the customer's `statecode` (via `projection._state_of`) against the target's state and carries it onto the DA component, so a state-only change lands `MERGED` (not a false `UNCHANGED`) and a state change rides along with a payload merge. The `ca-baseline.json` baseline does not record state, so the comparison is against the DA target's shipped state; the change is always surfaced in the result detail for verification.

**Action ordering (`_merge_list`).** A Power Fx action list is imperative, so order is load-bearing: an action the customer inserted *before* a terminal action (e.g. `EndDialog`) must stay before it. `_merge_list` first reconciles each action's *value* by `id` (inserts, deletes, edits told apart), then orders the result — starting from ESS's sequence and weaving each surviving customer-inserted action back in **after the nearest preceding action that still exists** (the anchor that encodes where the customer put it). If no preceding action survives and the insert was not at the very front, its position cannot be preserved, so it raises an ordering **conflict** rather than silently appending it to the end.

**Complete customer version on conflict (`ComponentResult.customer_version`).** Keeping ESS's component whole is the safe default for a `CONFLICTED` component, but `conflicts` only names the *contested* nodes — any edit the customer made that merged cleanly is discarded with the dropped merge and would otherwise vanish from both package and report. So a `CONFLICTED` result also carries `describe(component)` — the customer's complete version — which the report reproduces, guaranteeing every unapplied edit is on record for manual re-application.
